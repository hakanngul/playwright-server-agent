...
export class SystemMonitor extends EventEmitter {
  ...
  async _checkResources() {
    try {
      ...
      // Process bellek kullanımı (daha doğru)
      const processMemory = process.memoryUsage();
      const heapUsed = processMemory.heapUsed;
      const heapTotal = processMemory.heapTotal;
      const rss = processMemory.rss; // Resident Set Size

      // Process bellek kullanımı yüzdesi (RSS / Toplam Bellek)
      const processMemoryUsage = (rss / totalMem) * 100;

      // Daha gerçekçi bir bellek kullanımı değeri için process kullanımını kullan
