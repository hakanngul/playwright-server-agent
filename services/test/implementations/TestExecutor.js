...
import { ITestExecutor } from '../../core/ITestExecutor.js';
import { BrowserFactory } from '../../factory/BrowserFactory.js';
import { TestFactory } from '../../factory/TestFactory.js';
import { PerformanceFactory } from '../../factory/PerformanceFactory.js';

/**
 * Implements test executor interface
...
export class TestExecutor {
  ...
  constructor(options = {}) {
    ...
    this.testReporter = options.testReporter || TestFactory.createTestReporter({
    ...
    this.performanceReporter = PerformanceFactory.createPerformanceReporter({
      reportsDir: this.performanceReportsDir,
      thresholds: options.performanceThresholds
    });

    console.log(`TestExecutor created with browserType: ${this.browserType}, headless: ${this.headless}`);
  }
  ...
  async initialize() {
    ...
    if (!this.stepExecutor) {
      ...
      const elementHelper = BrowserFactory.createElementInteractor(page, this.screenshotsDir);
      const screenshotManager = BrowserFactory.createScreenshotManager(page, this.screenshotsDir);

      // Create step executor with dependencies
      this.stepExecutor = TestFactory.createStepExecutor(
        page,
        this.screenshotsDir,
      ...
      if (this.collectPerformanceMetrics) {
        this.performanceCollector = PerformanceFactory.createPerformanceCollector(page);
        this.networkMonitor = PerformanceFactory.createNetworkMonitor(page);

        // Set up performance observers
        await this.performanceCollector.setupPerformanceObservers();
        console.log('Performance monitoring initialized');
      }
    }

    console.log('TestExecutor initialized');
  }
  ...
  async runTest(testPlan) {
    ...
    if (this.collectPerformanceMetrics && this.performanceCollector) {
      try {
        console.log('Collecting Web Vitals and network metrics...');

        // Capture all performance metrics (Web Vitals and network metrics)
        const performanceData = await this.performanceCollector.captureAllMetrics(this.networkMonitor);
    ...
    try {
      const reportId = await this.testReporter.generateReport(results);
  ...
  setStepCompletedCallback(callback) {
    ...
    // Update the step executor if it exists
    if (this.stepExecutor) {
      this.stepExecutor.onStepCompleted = callback;
    }
  }

  /**
   * Closes the test executor and releases resources
   * @returns {Promise<void>}
   */
  async close() {
    console.log('Closing test executor and releasing resources...');
