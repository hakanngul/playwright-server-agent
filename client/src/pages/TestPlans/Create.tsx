...
const CreateTestPlan: React.FC = () => {
  ...
  const handleSaveTestPlan = async () => {
    ...
    try {
      ...
      // await fetch('http://localhost:3002/api/test-plans', {
      //   method: 'POST',
      //   headers: {
      ...
      // Başarılı kayıt sonrası test planları sayfasına yönlendir
      navigate('/test-plans');
    } catch (err) {
      setError('Test planı kaydedilirken bir hata oluştu.');
      console.error(err);
    } finally {
      setSaving(false);
    }
  ...
  return (
    <>
      <PageMeta
        title="Yeni Test Planı | Playwright Server Agent"
        description="Yeni test planı oluşturun"
      />
